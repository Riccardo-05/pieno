...
2. **Scheda principale.** Vetro bianco 72%, raggio 36 px, margine laterale 18 px. Contiene sempre e solo: nome, via, distanza, prezzo, risparmio, azione. La distanza è dichiarata **«in linea d'aria»**: è calcolata sulle coordinate e diverge dai km che il navigatore mostrerà un istante dopo. Il valore stradale arriva col servizio percorsi (`10-percorsi-e-backend.md`): quando c'è, la distanza mostrata è quella reale; quando il server non risponde si torna alla stima e lo si dichiara.
...
4. **Ricerca.** Raggio e ordinamento. *«Escludi dati più vecchi di» **non c'è più**: la soglia sui listini abbandonati la applica già la pipeline (regola R5, 30 giorni), quindi quelli non arrivano nemmeno all'app. Era una manopola tecnica su una decisione già presa a monte, e poteva stringere più della fonte facendo sparire impianti validi. L'attendibilità resta dichiarata dove serve: l'età del dato è sempre visibile.*
