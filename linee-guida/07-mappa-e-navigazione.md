...
- **Toccando un marcatore** l'impianto viene portato al centro della fascia di mappa che resterà scoperta, e il foglio torna all'altezza di riposo — esattamente come selezionandolo dall'elenco. *(Il documento di progetto prevedeva il contrario: muovere la mappa solo se il punto era fuori schermo, per non farlo «scappare sotto il dito». Cambiato perché il caso frequente è l'opposto — tocchi una pillola nella metà bassa dello schermo e il foglio, aprendosi, te la copre proprio mentre stavi guardando quella. Costo accettato: la pillola toccata si sposta.)*
...
- **Box.** Il pannello: tre altezze, 0,30 · **0,52** (riposo) · 0,92 di schermo. La **maniglia è chrome del box**, non un elemento della lista: non scorre via col contenuto. *(Il documento di progetto indicava 0,46: alzata perché la scheda dell'impianto è cresciuta — porta la distanza su strada col tempo di percorrenza, arrivata col servizio percorsi, e le stelle di valutazione. La misura è `kFoglioMedio` in `state/app_state.dart`, definita **una volta sola**: la leggono l'altezza del foglio e il calcolo del centro visibile della mappa. Quando era scritta due volte le copie sono divergute, e i comandi flottanti si sono ritrovati dentro il foglio.)*

- **I comandi flottanti chiedono l'altezza al foglio, non a una copia.** Attribuzione e pulsanti tondi si posizionano su `_sheetController.size`, cioè sulla posizione vera del pannello, non su un provider che la ricopia. Un valore ricopiato ha due modi di sbagliare — parte da un'altezza iniziale che può non corrispondere, e si aggiorna un fotogramma dopo, perché la notifica del foglio viaggia durante il layout — ed entrambi si vedono allo stesso modo: il pulsante «centra la posizione» che finisce sopra la scheda o ci sparisce dietro.
